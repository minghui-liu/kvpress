...
# Number of GPUs requested (must match --gres=gpu:N above)
NUM_GPUS=4

# Sweep settings
PRESS_NAME=("rkv" "h2o" "knorm" "snapkv" "streaming_llm")  # "full" "rkv" "h2o" "knorm" "snapkv" "streaming_llm"
MODELS=(
...
# =====================
# Verify multi-GPU is available
# =====================
NUM_VISIBLE=$(python -c "import torch; print(torch.cuda.device_count())")
echo "Visible GPUs: $NUM_VISIBLE (requested: $NUM_GPUS)"
if [[ "$NUM_VISIBLE" -lt "$NUM_GPUS" ]]; then
  echo "ERROR: Expected $NUM_GPUS GPUs but only $NUM_VISIBLE visible. Check --gres allocation."
  exit 1
fi
