...
def main():
    ...
    parser.add_argument(
        "--models", "--model",  # Accept both --model and --models
        type=str,
        nargs="+",
        default=[
        ...
        help="List of model names to process. Accepts both '/' and '--' formats."
    )
    parser.add_argument(
        "--results_dir",
        type=str,
        default="results",
        help="Directory containing step_tracking.json files (default: results)"
    )
    ...
    # Normalize model names: accept both "/" and "--" formats
    target_model_list = [m.replace("/", "--") for m in args.models]
    question_index = args.question_index
    
    # load all step_tracking files from results directory
    ...
    for step_tracking_file in step_tracking_files:
        for target_model in target_model_list:
            # Check if this file matches the criteria (model name uses -- in filenames)
            if target_model not in step_tracking_file.name:
                continue
            if target_press_name not in step_tracking_file.name:
                continue
            if f"budget{target_budget}" not in step_tracking_file.name:
                continue
