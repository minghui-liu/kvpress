#!/bin/bash
#SBATCH --job-name=scope_rpc_retention
...
# Separate dir so we do NOT collide with / skip the existing track_tokens=false runs.
RESULT_DIR="reason/results_retention"
mkdir -p logs "$RESULT_DIR"

# Sweep settings: SCOPE + RPC critical-token retention on GSM8K, all four models.
PRESS_NAME=("scope" "rpc")
MODELS=(
...
LAMBDA=0
N_HASH_BUCKETS=8
SNAPKV_WINDOW_SIZE=(128)   # dummy dimension (unused by scope/rpc), keeps index math intact

NUM_SAMPLES=100
RANDOM_SEEDS=(42)          # retention is ~seed-insensitive; add 24 130 for more samples
BLOCK_SIZE=20
...
# TOTAL should equal the --array upper bound + 1. With the defaults above:
#   NUM_SPECS = 4 models * 1 dataset * 2 presses * 4 budgets = 32
#   TOTAL     = 32 * 1 seed * 5 blocks * 1 window = 160  -> array 0-159
echo "TOTAL JOBS = $TOTAL (array should be 0-$((TOTAL - 1)))"
...
echo "➡️  Running $DATASET | press=$PRESS_METHOD | budget=$CACHE_BUDGET | max_new_tokens=$MAX_NEW_TOKENS | seed=$RANDOM_SEED | block=$DATASET_BLOCK_INDEX/${NUM_BLOCKS} | model=$MODEL_NAME"
python "$SCRIPT_PATH" \
...
echo "✅ Done $DATASET | press=$PRESS_METHOD | budget=$CACHE_BUDGET | max_new_tokens=$MAX_NEW_TOKENS | block=$DATASET_BLOCK_INDEX"
done
