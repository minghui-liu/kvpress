...
parser.add_argument("--save_name", type=str, default='mt_val_tracking', help='name of the save dataset jsonl file')
...
parser.add_argument("--initial_hops", type=int, default=5, help="Number of hops in the initial context")
parser.add_argument("--followup_hops", type=int, default=1, help="Number of hops in the followup context")
...
# templates
INITIAL_TEMPLATE = "You will be shown a block of text that contains variable assignments and updates. Memorize and keep track of the values of the variables. I will quiz you about them later.\n{content}"
FOLLOWUP_TEMPLATE = "Here is an additional block of text that contains variable assignments and updates. Memorize and keep track of the values of the variables. I will quiz you about them later.\n{content}"
QUESTION_TEMPLATE = "What are the value(s) of variable(s) {key}? Output the values of variable(s) {key}. Do not output anything else."
ANSWER_PREFIX = "The value(s) of {key} are: "
...
def add_keys(table, n_keys, n_hops=1):
    additions = {}
...
def update_keys(table, n_keys, n_hops=1):
    additions = {}
    keys = random.sample(list(table.keys()), n_keys)
    for key in keys:
        for _ in range(n_hops):
            value = generate_value()
            table[key] = value
...
def shuffle_kv_pairs(additions):
    strings = []
    candidates = list(additions.keys())
    while candidates:
        key = random.choice(candidates)
        value = additions[key].pop(0)
        strings.append(KV_PAIR_TEMPLATE.format(key=key, value=value))
...
def generate_initial_context():
    n_keys = args.initial_length // KV_PAIR_LEN_ESTIMATE // (args.initial_hops + 1)
    table, additions = add_keys({}, n_keys, args.initial_hops)
    strings = shuffle_kv_pairs(additions)
    return INITIAL_TEMPLATE.format(content=' '.join(strings)), table


def generate_followup_context(table):
    table, additions = update_keys(table, args.followup_length // KV_PAIR_LEN_ESTIMATE // args.followup_hops , args.followup_hops)
    strings = shuffle_kv_pairs(additions)
