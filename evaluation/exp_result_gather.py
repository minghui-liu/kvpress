...
subdir = "CONTEXT_SIZE"
datasets = ["mt_niah_S", "mt_niah_M", "mt_niah_L", "mt_vt_S", "mt_vt_M", "mt_vt_L", "mt_pr_S", "mt_pr_M", "mt_pr_L"]
...
for dataset in datasets:
    dataset_type = re.search(r"mt_(\w+)_\w+", dataset).group(1)
    dataset_type = dataset_type.upper()
    for press_name in press_names:
        score_filename = args.results_dir / subdir / dataset_type / (
...
def plot_task(task_name):
    ...
    save_dir = args.results_dir / "COLM_PLOTS" / "CONTEXT_SIZE"
    if not save_dir.exists():
        save_dir.mkdir(parents=True)
    plt.savefig(save_dir / f"{task_name}.png")
    print(f"Saved plot for {task_name} in {save_dir}")
    ...
    for press_name in press_names:
        for compression_ratio in compression_ratios:
            ...
            if compression_ratio == 0.5:
                score_filename = args.results_dir / subdir_50_cr / dataset_type / (
                    "__".join([dataset, model.replace("/", "--"), press_name, str(compression_ratio), "score"])
...
def cr_plot_task_press(task_name, press_name):
    ...
    save_dir = args.results_dir / "COLM_PLOTS" / "COMP_RATIO"
    if not save_dir.exists():
        save_dir.mkdir(parents=True)
    plt.savefig(save_dir/ f"{task_name}_{press_name}.png")
    print(f"Saved plot for {task_name} with {press_name} in {save_dir}")
...
for nquestions in [10, 20, 30, 40, 50]:
    for dataset in datasets:
        ...
        score_filename = args.results_dir / subdir / (
            "__".join([dataset_name, model.replace("/", "--"), press_name, str(compression_ratio), "score"])
            + ".json")
        if nquestions == 10:
            dataset_name = dataset
            score_filename = args.results_dir / subdir_10 / dataset_type / (
                "__".join([dataset_name, model.replace("/", "--"), press_name, str(compression_ratio), "score"])
                + ".json")
...
#--------- Create Plots ----------#
# Plot 3: Number of questions experiment
def nq_plot_task(task_name):
    # filter the dataframe
    task_df = nq_df[nq_df['dataset'] == task_name]
    task_df = task_df[task_df['compression_ratio'] == compression_ratio]
    # plot string match of scores in a line plot
    # for 5 different number of questions
    plt.figure(figsize=(12, 6))
    sns.set_theme(style="whitegrid")
    sns.set_context("paper", font_scale=1.5)
    ...
    plt.ylabel("String Match")
    plt.xlabel("Question Number")
    plt.legend(title='Number of Questions')
    save_dir = args.results_dir / "COLM_PLOTS" / "QUESTION_NUM"
    if not save_dir.exists():
        save_dir.mkdir(parents=True)
    plt.savefig(save_dir / f"{task_name}.png")
    print(f"Saved plot for {task_name} in {save_dir}")
